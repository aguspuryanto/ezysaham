...
function parseRssXml(xmlText: string, ticker: string): StockNewsItem[] {
  ...
  const parsed = matches
    ...
    .map((itemStr, idx) => {
      ...
      return {
        ...
        item: {
          ...
          url,
          publisher,
          publishedAt: publishedDate.toLocaleDateString('id-ID', {
            day: 'numeric',
            month: 'short',
            year: 'numeric',
            hour: '2-digit',
    ...
    .filter((entry): entry is { publishedAtMs: number; item: StockNewsItem } => entry !== null);

  // Google News RSS mixes relevance with recency, so sort explicitly by newest first.
  parsed.sort((a, b) => b.publishedAtMs - a.publishedAtMs);
...
function generateFallbackNews(ticker: string): StockNewsItem[] {
  ...
  return withDaysAgo.sort((a, b) => a.daysAgo - b.daysAgo).map((entry) => entry.item);
}
